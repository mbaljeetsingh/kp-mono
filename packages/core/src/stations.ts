 ...
 * Manmad answers with `icy-name: SikhNet Radio - Channel 5 - Siri Akhand Path`
 * — the mount is crossed with the Akhand Path channel upstream, so tapping that
 * gurdwara plays something else entirely under its name. Wrong audio behind a
 * right name is worse than an absence.
 */
import stations from '@kp/shared/data/stations.json' with { type: 'json' };
...
/**
 * Programmed 24-hour feeds — katha, simran, the unbroken reading. Nothing here
 * overlaps the archive; they are the material this project does not have.
 */
export const CHANNELS = STATIONS.filter((s) => s.kind === 'channel');

/**
 * What the Radio control starts, and the one feed served by the organisation
 * that recorded the archive.
 */
 ...
 */
export const OTHER_GURDWARAS = GURDWARAS.filter((s) => s.id !== DEFAULT_STATION.id);

/**
 * Namespaced so a station id can never collide with a segment uuid, and so a
 * stray id in a log or in storage says what it is.
 */
